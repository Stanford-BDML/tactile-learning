...
class REINFORCEAgent(object):
    ...
    def _policy_nn(self):        
        hid1_size = self.hdim	# substitute the hidden layers size which is define in "init".(hdim=64)
        hid2_size = self.hdim
        
        # TWO HIDDEN LAYERS. input=obs_ph, hid1_size=16, activation function=tanh. two hidden layers(dence = fully connect)
        ...
        self.logits = tf.layers.dense(out, self.n_act,
                              kernel_initializer=tf.random_normal_initializer(stddev=0.01,seed= self.seed), name="logits")	#Logits layer(dense = FC). Output=n_act (return an estimated action)
        ...
        self.logstd = tf.Variable(tf.zeros([1, self.n_act]),
                                dtype=tf.float32, name="logstd") 	# define new variable named "logstd", size = [1, n_act]

        self.sample_action = self.output_placeholder + tf.exp(self.logstd) * tf.random_normal(tf.shape(self.output_placeholder)) # calculate sample_action. shape [2, ]

        # SOFTMAX POLICY
        self.pi = self.output_placeholder	# "pi" represent the same meaning with sample_action but it doesn't include random element
    ...
    def get_action(self, obs): # SAMPLE FROM POLICY
        feed_dict = {self.obs_ph: obs}  					# make the dictionary named "feed_dict" which is composed of obs place holder and input the observation
    ...
    def update(self, observes, actions, scores, batch_size = 128): # TRAIN POLICY        
        #print ('########### TRAIN POLICY ###############')
        num_batches = max(observes.shape[0] // batch_size, 1)	# calculate number of batches. //=切り捨て除算
